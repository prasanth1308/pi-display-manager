...
document
  ...
  .addEventListener("click", () => controlAction("stop"));
document
  .getElementById("btn-next")
  .addEventListener("click", () => controlAction("next"));
...
async function downloadYouTube() {
  ...
  try {
    ...
    progress.style.display = "block";
    fill.style.width = "50%"; // Indeterminate progress

    const result = await api.post("/api/files/download-youtube", { url });

    fill.style.width = "100%";
    toast(`Downloaded: ${result.original_name}`);
    urlInput.value = "";
    loadFiles();
    loadPlaylists();
  } catch (err) {
    ...
    <div class="playlist-row" draggable="true" data-idx="${idx}">
      ...
      <span class="row-duration">
        ${
          ...
          onchange="updateItemDuration(${idx}, this.value)" title="Display seconds"> s`
...
document.getElementById("file-picker-confirm").addEventListener("click", () => {
  const checked = [
    ...document.querySelectorAll("#file-picker-list input:checked"),
  ];
...
function renderFilePicker() {
  ...
  if (!allFiles.length) {
    list.innerHTML =
      '<p style="color:var(--muted);padding:20px;text-align:center">No files uploaded</p>';
    return;
  }
...
async function createSchedule() {
  ...
  const playlist_id = parseInt(
    document.getElementById("schedule-playlist").value,
  );

  if (!name || !cron || !playlist_id) {
